...
export const localOffer = offer => qrinv(offer).then(qr => ({ unitf, conf: { expert } }) =>
  div('.local-offer-recv', [
    div('.row', [
      ...
      //, p(`The BOLT12 payment code below is reusable and allows receiving multiple payments${offer.msatoshi != 'any'?` of ${unitf(offer.msatoshi)} each`:''}.`)
      //, p(`Reusable BOLT12 offer to receive multiple payments${offer.msatoshi != 'any'?` of ${unitf(offer.msatoshi)} each`:''}:`)
      , p(`Receive multiple payments${offer.msatoshi != 'any'?` of ${unitf(offer.msatoshi)} each`:''} using the reusable BOLT12 offer:`)
      //, offer.msatoshi !== 'any' ? h3('.toggle-unit', unitf(offer.msatoshi)) : ''
