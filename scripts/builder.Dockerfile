...
RUN apt-get update && apt-get install -y --no-install-recommends git=1:2.11.0-3+deb9u7 binutils=2.28-5 software-properties-common=0.96.20.2-1 \
...
RUN dpkg --add-architecture i386 \
  ...
  && apt-get update && apt-get install -y --no-install-recommends winehq-stable=5.0.1~stretch
...
WORKDIR /root
# Java 8 (OpenJDK)
RUN apt-get install -y --no-install-recommends openjdk-8-jdk-headless=8u252-b09-1~deb9u1
